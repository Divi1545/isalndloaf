...
const AnalyticsDashboard = () => {
  ...
  const { data: bookingAnalytics, isLoading: bookingLoading, refetch: refetchBookings } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch('/api/dashboard/booking-analytics');
  ...
  const bookingStatusData = Object.entries(bookingAnalytics?.statusDistribution || {}).map(([status, count]) => ({
    ...
    count: count,
    percentage: Math.round((count / Object.values(bookingAnalytics?.statusDistribution || {}).reduce((a, b) => a + b, 0)) * 100)
  }));
